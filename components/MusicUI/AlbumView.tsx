...
export default function AlbumView({ url, syncId, thinBackgroundColor, data }: AlbumViewProps) {
		...
		async function fetchAlbum() {
			...
			try {
				...
				if (!response.ok) {
					...
					setError(errorData.error || errorData.message || "Failed to load album data");
					setIsLoading(false);
					return;
				}
		...
		return (
			<div className="min-h-screen flex items-center justify-center bg-background">
				<div className="text-center p-8">
					<h1 className="text-2xl font-bold text-foreground mb-4">Error Loading Album</h1>
					...
					<div className="flex flex-col md:flex-row gap-6 mb-8">
						...
						<div className="flex-1 text-center md:text-left">
							<motion.h1
								...
								className="text-3xl md:text-4xl font-bold text-white mb-3 drop-shadow-[0_4px_16px_rgba(0,0,0,0.8)]"
							>
								{album.title}
							</motion.h1>
							<motion.p
							...
							<motion.div
								...
								className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm text-white/90 mb-6 drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]"
							>
							...
							{uniqueSongs.map((song, index) => (
								...
								>
									<div className="shrink-0 w-8 text-center">
										<span className="text-sm text-white/70 group-hover:hidden drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]">
											{index + 1}
										</span>
