...
export const buildPreview = (data: PreviewSource | null | undefined): ComposerPreview | null => {
	...
	const conversionWarnings = (data as { conversionWarnings?: typeof data.conversionWarnings })
		.conversionWarnings;
	const normalizedType = typeof data.type === "string" ? data.type.toLowerCase() : undefined;

	const albumArtistList = Array.isArray(data.artists) ? data.artists.filter(Boolean) : [];

	if (
		normalizedType === "artist" ||
		((data as SyncFMArtist).name && (!data.title || normalizedType === "artist"))
	) {
		...
		const supporting = releases > 0 ? `${releases} release${releases > 1 ? "s" : ""}` : undefined;
	...
	const isPlaylist =
		...
		("originalService" in data && Array.isArray((data as SyncFMPlaylist).songs));
	const isAlbum =
		!isPlaylist &&
		(normalizedType === "album" || "songs" in data || typeof data.totalTracks === "number");
	...
	if (isPlaylist || isAlbum) {
		...
		if (totalTracks) {
			supportingParts.push(`${totalTracks} track${totalTracks === 1 ? "" : "s"}`);
		}
	...
	const artists = Array.isArray(song.artists) ? song.artists.join(", ") : undefined;
