...
import { buildPreview, decodeLinkMetadata } from "./utils";
...
const LinkComposer = ({
	...
	const handleSubmit = useCallback(
		async (event: React.FormEvent<HTMLFormElement>) => {
			...
			if (trimmed.includes("syncfm.dev")) {
				setError("That's already a SyncFM link! Drop the original streaming URL instead.");
				setStatus("error");
				return;
			}
			...
			try {
				const response = await fetch(`/api/handle/syncfm?url=${encodeURIComponent(trimmed)}`);
				...
				const hasWarnings =
					built?.conversionWarnings && Object.keys(built.conversionWarnings).length > 0;
				...
				if (built) {
					setDecodedMeta((current) => (current ? { ...current, typeLabel: built.type } : current));
				}
			} catch (fetchError) {
	...
	const handlePaste = useCallback(async () => {
		...
		try {
			...
			setError("We couldn't access your clipboard. Try pasting manually into the field.");
			setStatus("error");
		} finally {
			setIsPasting(false);
		}
		...
		<>
			...
			<AnimatePresence>
				{expanded && (
					<motion.div
						key="composer-modal"
						className="fixed inset-0 z-50 flex min-h-screen items-center justify-center bg-background/85 p-4 backdrop-blur-xl sm:p-8"
						initial={{ opacity: 0 }}
						animate={{ opacity: 1 }}
						exit={{ opacity: 0 }}
