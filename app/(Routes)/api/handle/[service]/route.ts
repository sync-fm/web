...
export async function GET(
	...
	try {
		...
		if (syncId) {
			...
			try {
				...
				if (!noRedirect) {
					switch (inputType) {
						case "song":
							convertedUrl = await syncfm.createSongURL(
								convertedData as SyncFMSong,
								service,
								syncId
							);
							break;
						case "album":
							convertedUrl = await syncfm.createAlbumURL(
								convertedData as SyncFMAlbum,
								service,
								syncId
							);
							break;
						case "artist":
							convertedUrl = await syncfm.createArtistURL(
								convertedData as SyncFMArtist,
								service,
								syncId
							);
							break;
		...
		try {
			switch (inputType) {
				case "song":
					convertedData = await syncfm.convertSong(
						await syncfm.getInputSongInfo(originalUrl),
						service
					);
					...
					break;
				case "album":
					convertedData = await syncfm.convertAlbum(
						await syncfm.getInputAlbumInfo(originalUrl),
						service
					);
					if (!noRedirect && convertedData) {
						convertedUrl = await syncfm.createAlbumURL(
							convertedData,
							service,
							convertedData.syncId
						);
					}
					break;
				case "artist":
					convertedData = await syncfm.convertArtist(
						await syncfm.getInputArtistInfo(originalUrl),
						service
					);
					if (!noRedirect && convertedData) {
						convertedUrl = await syncfm.createArtistURL(
		...
		if (!convertedData) {
			...
			if (!noRedirect) {
				...
				errorUrl.searchParams.set(
					"message",
					`${inputType.charAt(0).toUpperCase() + inputType.slice(1)} not found or unavailable on ${service}`
				);
				return respondRedirect(errorUrl, analytics);
			}
