...
export default class YTMusic {
	...
	public async getSong(videoId: string): Promise<SongFull> {
		...
		let data;
		try {
			data = await this.constructRequest("player", { videoId })
			console.log("Raw song data:", data)
		} catch (error) {
			throw new Error("Error fetching song data: " + error)
		}
		let song;
		try {
			song = SongParser.parse(data)
			console.log("Parsed song data:", song)
		} catch (error) {
			throw new Error("Error parsing song data: " + error)
		}
