// biome-ignore-all lint/style/noParameterAssign: stfu
"use server";
export async function getDominantColorFromImageUrl(imageUrl?: string): Promise<string | null> {
	...
	try {
		...
		const [r, g, b] = sorted[0][0].split(",").map((v) => Number.parseInt(v, 10));
 ...
 */
export async function thinBackgroundFromHex(hex: string | null | undefined): Promise<string> {
